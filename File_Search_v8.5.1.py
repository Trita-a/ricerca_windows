...
class FileSearchApp:
    def __init__(self, root):
        ...
        self.system_file_extensions = [
            ...
            '.bin', '.dat', '.db', '.sqlite', '.sqlite3', '.qvm', '.pak', '.idx', '.wim', '.vmdk', '.qcow2', '.mdb', '.accdb',
    ...
    def show_content_search_warning(self):
        ...
        if self.search_content.get():  # Se la checkbox è attivata
            # Verifica se stiamo cercando su un'unità di sistema
            path = self.search_path.get()
            is_system_path = path.lower() in ["c:/", "c:\\", "d:/", "d:\\", "e:/", "e:\\"]
            
            warning_message = "Hai attivato la ricerca nei contenuti dei file.\n\n"
            
            if is_system_path:
                warning_message += "ATTENZIONE: Stai cercando su un'intera unità di sistema!\n" \
                                "Questa operazione può richiedere MOLTO tempo (ore) e " \
                                "potrebbe rallentare temporaneamente il computer.\n\n"
            else:
                warning_message += "Questa operazione può richiedere molto più tempo, " \
                                "soprattutto con grandi quantità di file.\n\n"
            
            warning_message += "Vuoi procedere con la ricerca nei contenuti?"
            
            # Aggiungere la personalizzazione con data/utente
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            current_user = getpass.getuser()
            
            footer = f"\n\nUtente: {current_user} | Data: {current_time}"
            warning_message += footer
    ...
    def create_widgets(self):
        ...
        ttk.Button(options_row2, text="Filtri avanzati", 
                command=self.show_advanced_filters_dialog, width=15).pack(side=LEFT, padx=5)

        # Pulsante per gestire le esclusioni spostato nella seconda riga
        ttk.Button(options_row2, text="Gestisci esclusi", 
                command=self.manage_exclusions, width=15).pack(side=LEFT, padx=5)
        ...
        if os.name == 'nt':
            admin_button = ttk.Button(button_frame, 
            ...
            if self.is_admin:
                ...
                self.create_tooltip(admin_button, "L'applicazione è già in esecuzione come amministratore")
            else:
                self.create_tooltip(admin_button, "Riavvia l'applicazione con privilegi di amministratore per accedere a tutte le cartelle")
