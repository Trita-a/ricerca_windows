...
class FileSearchApp:
    def __init__(self, root):
        self.root = root
        self.root.title("File Search Tool V8.7.6. Nucleo Perugia")
        ...
        # Posticipa controlli non essenziali all'avvio
        self.root.after(1000, self._delayed_startup_tasks)

    def _init_variables(self):
        """Inizializza tutte le variabili dell'applicazione"""
    ...
    def _delayed_startup_tasks(self):
        """Esegue attività non essenziali con ritardo per velocizzare l'avvio"""
        ...
        # Aggiorna informazioni disco
        self.update_disk_info(calculate_dir_size=False)
    ...
    def _check_available_libraries(self):
        ...
        # Mostra notifica dopo un breve ritardo per non disturbare l'avvio
    ...
    def should_search_content(self, file_path):
        ...
        # Verifica rapida per file di sistema
        if self.exclude_system_files.get() and ext in self.system_file_extensions:
            return False
        
        # Se l'esclusione è disattivata, accetta tutti i file
        if not self.exclude_system_files.get():
            return True
        
        # Usa un set per ricerche più veloci
        text_extensions = {'.txt', '.md', '.csv', '.html', '.htm', '.xml', '.json', '.log'}
        if ext in text_extensions:
            return True
            
        # Verifica altre estensioni supportate
        if (ext in {'.docx'} and file_format_support["docx"]) or \
    ...
    def get_file_content(self, file_path):
        ...
        try:
            ...
            # File di testo semplice - ottimizzato per caricamento rapido
            ...
            # Word (.docx) - Importa modulo solo se necessario
            ...
            # PDF - Importa modulo solo se necessario
            ...
            # Per altri formati, tenta l'apertura come testo se l'esclusione file di sistema è disattivata
            if not self.exclude_system_files.get():
                try:
                    with open(file_path, 'rb') as f:
            ...
            self.log_debug(f"Errore generale nella lettura del file: {str(e)}")
            return ""
