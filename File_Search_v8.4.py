        ...
        try:
           ...
           # Determina se si tratta di una ricerca completa del sistema (C:/ o simile)
            ...
            def visit_directory(directory, current_depth=0):
                ...
                # MODIFICATO: migliorata la gestione dei percorsi per evitare loop infiniti
                try:
                ...
                try:
                    ...
                    # Aggiorna lo stato più frequentemente su server
                    if dirs_checked % 5 == 0:  # Modificato da 10 a 5
                    ...
                    # MODIFICATO: gestione più robusta dei listdir
                    try:
                    ...
                    # MODIFICATO: separato il loop per rendere più chiaro il codice
                    for item in items:
                        if self.stop_search:
                            return
                        ...
                        # MODIFICATO: gestione più robusta degli errori per ogni item
                        try:
                    ...
                    # MODIFICATO: processo dei file con miglior gestione degli errori
                    for item in items:
                        if self.stop_search:
                            return
                        ...
                        try:
                            # Controllo se è una cartella nascosta
                            if self.ignore_hidden.get() and (item.startswith('.') or 
                                                (os.name == 'nt' and os.path.exists(item_path) and 
                                                    os.stat(item_path).st_file_attributes & 2)):
                                continue
                        ...
                        if self.search_files.get():
                            try:
                                ...
                                # Aggiorna progresso più frequentemente
                                if files_checked % 25 == 0:  # Modificato da 50 a 25
                                    elapsed_time = time.time() - start_time
                                    self.progress_queue.put(("status", 
                    ...
                    # MODIFICATO: chiamate ricorsive con gestione degli errori
                    for dir_path in directories:
                        if self.stop_search:
                            return
                    ...
                    for dir_path in directories:
                        ...
                        try:
                            ...
                            # Non fermare la ricerca se una directory causa problemi
                            self.log_debug(f"Errore nell'esplorazione di {dir_path}: {str(e)}")
                            continue
