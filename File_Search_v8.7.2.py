...
class FileSearchApp:
    ...
    def create_widgets(self):
       ...
       # Header con titolo, tema e info utente tutto in un'unica riga
        ...
        # Frame for disk information
        disk_info_frame = ttk.LabelFrame(right_frame, text="Informazioni spazio disco")
        disk_info_frame.pack(fill=X, pady=5, padx=5)

        # Grid for disk information
        disk_grid = ttk.Frame(disk_info_frame)
        disk_grid.pack(fill=X, padx=5, pady=5)

        # Directory size
        ttk.Label(disk_grid, text="Dimensione percorso:").grid(row=0, column=0, sticky=W, padx=5, pady=2)
        ttk.Label(disk_grid, textvariable=self.dir_size_var, font=("", 9, "bold")).grid(row=0, column=1, sticky=W, padx=5, pady=2)

        # Total disk space
        ttk.Label(disk_grid, text="Dimensione disco:").grid(row=0, column=2, sticky=W, padx=(15,5), pady=2)
        ttk.Label(disk_grid, textvariable=self.total_disk_var, font=("", 9, "bold")).grid(row=0, column=3, sticky=W, padx=5, pady=2)

        # Used space
        ttk.Label(disk_grid, text="Spazio utilizzato:").grid(row=1, column=0, sticky=W, padx=5, pady=2)
        ttk.Label(disk_grid, textvariable=self.used_disk_var, font=("", 9, "bold")).grid(row=1, column=1, sticky=W, padx=5, pady=2)

        # Free space
        ttk.Label(disk_grid, text="Spazio libero:").grid(row=1, column=2, sticky=W, padx=(15,5), pady=2)
        ttk.Label(disk_grid, textvariable=self.free_disk_var, font=("", 9, "bold")).grid(row=1, column=3, sticky=W, padx=5, pady=2)
