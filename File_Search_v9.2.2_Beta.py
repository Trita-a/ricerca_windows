...
class FileSearchApp:
    ...
    def disable_all_controls(self):
        ...
        try:
            ...
            self.safe_set_state('path_entry', "disabled")
            self.safe_set_state('keyword_entry', "disabled")
            
            # Pulsante Sfoglia
            self.safe_set_state('browse_btn', "disabled")
            self.safe_set_state('clear_btn', "disabled")
            
            # Combobox e spinbox
            if hasattr(self, 'theme_combobox'):
                self.safe_set_state('theme_combobox', "disabled")
            
            if hasattr(self, 'depth_spinbox'):
                self.safe_set_state('depth_spinbox', "disabled")
            
            # Pulsanti principali
            self.safe_set_state('search_button', "disabled")
            
            # Gestisci i pulsanti dei filtri
            for btn_name in ['advanced_filters_btn', 'exclusions_btn', 'block_options_btn']:
                if hasattr(self, btn_name):
                    self.safe_set_state(btn_name, "disabled")
            
            # Gestisci specificamente il pulsante admin
            if hasattr(self, 'admin_button') and not self.is_admin:
                self.safe_set_state('admin_button', "disabled")

            # Disabilita il pulsante delle opzioni di performance
            if hasattr(self, 'perf_options_btn'):
                self.safe_set_state('perf_options_btn', "disabled")
            
            # Pulsanti di azione sui risultati
            self.safe_set_state('copy_button', "disabled")
            self.safe_set_state('compress_button', "disabled")
    ...
    def enable_all_controls(self):
        ...
        # Input e campi di testo
        self.safe_set_state('path_entry', "normal")
        self.safe_set_state('keyword_entry', "normal")
        
        # Pulsante Sfoglia
        self.safe_set_state('browse_btn', "normal")
        
        # Pulsante Pulisci campi
        self.safe_set_state('clear_btn', "normal")
        
        # Combobox e spinbox
        if hasattr(self, 'theme_combobox'):
            self.safe_set_state('theme_combobox', "readonly")
        
        if hasattr(self, 'depth_spinbox'):
            self.safe_set_state('depth_spinbox', "normal")
        
        # Pulsanti principali
        self.safe_set_state('search_button', "normal")
        
        # Gestisci i pulsanti dei filtri
        for btn_name in ['advanced_filters_btn', 'exclusions_btn']:
            if hasattr(self, btn_name):
                self.safe_set_state(btn_name, "normal")

        # Gestisci i pulsanti a blocchi
        if hasattr(self, 'block_options_btn'):
            self.safe_set_state('block_options_btn', "normal")

        # Gestisci specificamente il pulsante admin
        if hasattr(self, 'admin_button') and not self.is_admin:
            self.safe_set_state('admin_button', "normal")
        ...
        # Pulsanti di azione sui risultati
        self.safe_set_state('copy_button', "normal")
        self.safe_set_state('compress_button', "normal")
        
        # Abilita il pulsante delle opzioni di performance
        if hasattr(self, 'perf_options_btn'):
            self.safe_set_state('perf_options_btn', "normal")
        ...
        try:
            if hasattr(self, 'depth_spinbox'):
                try:
                    ...
                    # Widget no longer exists or has an invalid value
                    if hasattr(self, 'depth_spinbox'):
                        delattr(self, 'depth_spinbox')  # Remove the invalid reference
                    # Keep using the existing self.max_depth value
            # If depth_spinbox doesn't exist, keep using the existing self.max_depth value
        except ValueError:
            self.max_depth = 0  # Default value if invalid
    ...
    def safe_set_state(self, widget_attr, state):
        """Safely sets the state of a widget if it exists and is valid"""
        if hasattr(self, widget_attr):
            try:
                getattr(self, widget_attr)["state"] = state
                return True
            except (tk.TclError, AttributeError):
                # Widget no longer exists, remove the reference
                if hasattr(self, widget_attr):
                    delattr(self, widget_attr)
                return False
        return False
    ...
    def show_advanced_filters_dialog(self):
        ...
        dialog.geometry("530x350")  # Aumentato l'altezza per includere il nuovo campo
        dialog.transient(self.root)
        dialog.grab_set()
        ...
        # AGGIUNTO: Filtri estensione
        ext_frame = ttk.LabelFrame(dialog, text="Estensioni file")
        ext_frame.pack(fill=X, padx=10, pady=5)
        
        ttk.Label(ext_frame, text="Inserisci le estensioni separate da virgola (es: pdf,txt,docx):").pack(anchor=W, pady=(5, 5))
        
        # Variabile per contenere il valore del campo estensioni
        extension_var = StringVar()
        if "extensions" in self.advanced_filters and self.advanced_filters["extensions"]:
            # Rimuovi i punti e unisci con virgole
            extensions_text = ",".join([ext.lstrip('.') for ext in self.advanced_filters["extensions"]])
            extension_var.set(extensions_text)
        
        extensions = ttk.Entry(ext_frame, width=50, textvariable=extension_var)
        extensions.pack(fill=X, padx=10, pady=5)
        ...
        def save_filters():
            try:
                ...
                # Analizza le estensioni - ORA FUNZIONA PERCHÉ extensions È DEFINITO
    ...
    def show_advanced_options(self):
        ...
        # Local variables to store settings temporarily
        local_settings = {
            "max_depth": self.max_depth,
            "search_files": self.search_files.get(),
            "search_folders": self.search_folders.get(),
            "search_content": self.search_content.get(),
            "whole_word_search": self.whole_word_search.get(),
            "advanced_filters": {
                "size_min": self.advanced_filters["size_min"],
                "size_max": self.advanced_filters["size_max"],
                "date_min": self.advanced_filters["date_min"],
                "date_max": self.advanced_filters["date_max"],
                "extensions": self.advanced_filters["extensions"].copy() if "extensions" in self.advanced_filters else []
            },
            "excluded_paths": self.excluded_paths.copy() if hasattr(self, 'excluded_paths') else [],
            "worker_threads": self.worker_threads.get(),
        ...
        # Create a local StringVar for depth_spinbox
        depth_var = StringVar(value=str(local_settings["max_depth"]))
        ...
        # Create local variables for checkbuttons
        search_files_var = BooleanVar(value=local_settings["search_files"])
        search_folders_var = BooleanVar(value=local_settings["search_folders"])
        search_content_var = BooleanVar(value=local_settings["search_content"])
        whole_word_var = BooleanVar(value=local_settings["whole_word_search"])
        ...
        min_size_var = StringVar(value=str(local_settings["advanced_filters"]["size_min"] // 1024))
        ...
        max_size_var = StringVar(value=str(local_settings["advanced_filters"]["size_max"] // 1024 if local_settings["advanced_filters"]["size_max"] else 0))
        ...
        min_date.grid(row=0, column=1, padx=5, pady=5)
        min_date.entry.delete(0, 'end')
        if local_settings["advanced_filters"]["date_min"]:
            min_date.entry.insert(0, local_settings["advanced_filters"]["date_min"])
        ...
        max_date.entry.delete(0, 'end')
        if local_settings["advanced_filters"]["date_max"]:
            max_date.entry.insert(0, local_settings["advanced_filters"]["date_max"])
        
        # Estensioni
        ext_frame = ttk.LabelFrame(filters_frame, text="Estensioni file", padding=10)
        ext_frame.pack(fill=X, pady=10)
        
        ttk.Label(ext_frame, text="Inserisci le estensioni separate da virgola (es: pdf,txt,docx):").pack(anchor=W, pady=(0, 10))
        
        extension_var = StringVar()
        if local_settings["advanced_filters"]["extensions"]:
            # Remove dots and join
            extensions_text = ",".join([ext.lstrip('.') for ext in local_settings["advanced_filters"]["extensions"]])
            extension_var.set(extensions_text)
        
        extensions = ttk.Entry(ext_frame, width=50, textvariable=extension_var)
        extensions.pack(fill=X, pady=5)
        ...
        for path in local_settings["excluded_paths"]:
            excluded_list.insert("", "end", values=(path,))
        ...
        block_size_auto_var = BooleanVar(value=self.block_size_auto_adjust.get())
        ttk.Checkbutton(options_frame, text="Adatta automaticamente la dimensione dei blocchi", 
                    variable=block_size_auto_var).pack(anchor=W, pady=2)
        ...
        timeout_var = BooleanVar(value=self.timeout_enabled.get())
        timeout_check = ttk.Checkbutton(timeout_grid, text="Attiva timeout ricerca", variable=timeout_var)
        ...
        ttk.Label(process_grid, text="Thread paralleli:").grid(row=0, column=0, sticky=W, padx=5, pady=5)
        threads_var = IntVar(value=local_settings["worker_threads"])
        ...
        max_file_size_var = IntVar(value=local_settings["max_file_size_mb"])
        max_size_mb = ttk.Spinbox(process_grid, from_=1, to=1000, width=5, textvariable=max_file_size_var)
        ...
        dir_size_var = StringVar(value=local_settings["dir_size_calculation"])
        calc_combo = ttk.Combobox(calc_frame, textvariable=dir_size_var, 
                            values=["incrementale", "preciso", "stimato", "sistema", "disabilitato"], 
                            width=12, state="readonly")
        calc_combo.pack(side=LEFT, padx=5)
        
        # Function to safely get the values when saving
        ...
        # Function to gather all excluded paths from the treeview
        ...
        def save_options():
            try:
                # Update max_depth
                try:
                    self.max_depth = get_safe_int(depth_var.get(), 0)
                except Exception as e:
                    self.log_debug(f"Error getting depth value: {str(e)}")
                    self.max_depth = local_settings["max_depth"]  # Keep original value
                ...
                try:
                    # Size filters
                    min_kb = get_safe_int(min_size_var.get(), 0)
                    max_kb = get_safe_int(max_size_var.get(), 0)
                    self.advanced_filters["size_min"] = min_kb * 1024
                    self.advanced_filters["size_max"] = max_kb * 1024
                    ...
                    if min_date_val:
                        try:
                            datetime.strptime(min_date_val, "%d-%m-%Y")
                            self.advanced_filters["date_min"] = min_date_val
                        except ValueError:
                            self.log_debug(f"Invalid min date format: {min_date_val}")
                            # Keep original value
                    else:
                        self.advanced_filters["date_min"] = None
                    
                    if max_date_val:
                        try:
                            datetime.strptime(max_date_val, "%d-%m-%Y")
                            self.advanced_filters["date_max"] = max_date_val
                        except ValueError:
                            self.log_debug(f"Invalid max date format: {max_date_val}")
                            # Keep original value
                    else:
                        self.advanced_filters["date_max"] = None
                    
                    # Extensions
                    exts = [e.strip() for e in extension_var.get().split(",") if e.strip()]
                    self.advanced_filters["extensions"] = [f".{e.lstrip('.')}" for e in exts]
                    
                except Exception as e:
                    self.log_debug(f"Error updating advanced filters: {str(e)}")
                    # Keep original values for safety
                    
                # Update excluded paths
                self.excluded_paths = get_excluded_paths()
                
                # Update block options
                self.max_files_per_block.set(get_safe_int(files_block_var.get(), 1000))
                self.max_parallel_blocks.set(get_safe_int(parallel_var.get(), 4))
                self.block_size_auto_adjust.set(block_size_auto_var.get())
                self.prioritize_user_folders.set(prioritize_var.get())
                
                # Update performance options
                self.timeout_enabled.set(timeout_var.get())
                self.timeout_seconds.set(get_safe_int(timeout_seconds_var.get(), 3600))
                self.max_files_to_check.set(get_safe_int(max_files_var.get(), 100000))
                self.max_results.set(get_safe_int(max_results_var.get(), 50000))
                self.worker_threads.set(get_safe_int(threads_var.get(), 4))
                self.max_file_size_mb.set(get_safe_int(max_file_size_var.get(), 100))
                self.dir_size_calculation.set(dir_size_var.get())
                
                self.log_debug("Opzioni avanzate salvate con successo")
                
            except Exception as e:
                self.log_debug(f"Errore nel salvataggio delle opzioni avanzate: {str(e)}")
                messagebox.showerror("Errore", f"Si è verificato un errore durante il salvataggio delle opzioni:\n{str(e)}")
            
            finally:
                # Always destroy the dialog, even if there was an error
        ...
        def reset_defaults():
            ...
            # Clear excluded paths list
            for item in excluded_list.get_children():
                excluded_list.delete(item)
                
            files_block_var.set(1000)
            parallel_var.set(4)
            block_size_auto_var.set(True)
            prioritize_var.set(True)
            
            timeout_var.set(False)
            timeout_seconds_var.set(3600)
            max_files_var.set(100000)
            max_results_var.set(50000)
            threads_var.set(min(8, os.cpu_count() or 4))
            max_file_size_var.set(100)
            dir_size_var.set("disabilitato")
            
            messagebox.showinfo("Valori predefiniti", "I valori predefiniti sono stati ripristinati. Clicca Salva per applicarli.")
        
        ttk.Button(btn_frame, text="Ripristina valori predefiniti", command=reset_defaults).pack(side=LEFT)
        ttk.Button(btn_frame, text="Annulla", command=on_dialog_close).pack(side=RIGHT, padx=5)
