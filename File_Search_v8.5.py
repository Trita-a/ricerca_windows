...
class FileSearchApp:
    def __init__(self, root):
        ...
        self.system_file_extensions = [
            ...
            '.exe', '.dll', '.sys', '.drv', '.ocx', '.vxd', '.com', '.bat', '.cmd', '.scr',
            # File di configurazione
            '.ini', '.inf', '.reg', '.config', '.msi', '.msp', '.cab',
            # File di cache e temporanei
            '.tmp', '.temp', '.dmp', '.cache', '.bak', '.swp',
            # File di sistema Windows
            '.lnk', '.ico', '.cur', '.ani', '.cpl', '.evt', '.cat', '.mui',
            # File di sistema Linux
            '.so', '.o', '.a', '.la', '.ko', '.mod',
            # Altri file binari comuni
            '.bin', '.dat', '.db', '.sqlite', '.sqlite3',
            # File specifici di sistema
            '.efi', '.pf', '.etl', '.mui', '.res', '.tlb', '.pdb'
        ]
